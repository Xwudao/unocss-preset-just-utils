...
export const rings: Rule<Theme>[] = [
  ...
  [
    /^ring-(?:width-|size-)(.+)$/,
    ([, d], { theme }) => ({
      "--un-ring-width": theme.lineWidth?.[d] ?? h.bracket.cssvar.px(d),
    }),
  ...
  [
    ...
    ([, d], { theme }) => ({
      "--un-ring-offset-width": theme.lineWidth?.[d] ?? h.bracket.cssvar.px(d),
    }),
  ...
  [
    /^ring-op(?:acity)?-?(.+)$/,
    ([, opacity]) => ({
      "--un-ring-opacity": h.bracket.percent.cssvar(opacity),
    }),
  ...
  [
    ...
    ([, opacity]) => ({
      "--un-ring-offset-opacity": h.bracket.percent.cssvar(opacity),
    }),
